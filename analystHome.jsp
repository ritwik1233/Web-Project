...
try{
result1=stmt1.executeQuery("SELECT ID FROM USERS WHERE POWER=0");
...
<li><input type="button" class="btns" value="Search by Date"  onclick="openPage('analystSearch.jsp')"></li>
<li><input type="button" class="btns" value="Generate Request"  onclick="openPage('fileUpload.jsp')"></li>
</ul>
</div>
</div>
...
<ul>
<li>
<input type="button" class="btns" value="Pending with Leader" onclick="openPage('analystPending.jsp')" />
</li>
<li>
<input type="button" class="btns" value="Forwarded to Admin"  onclick="openPage('analystRaised.jsp')" />
</li>
<li>
<input type="button" class="btns" value="Assigned to Extractor"  onclick="openPage('analystAssigned.jsp')" />
</li>
<li><input type="button" class="btns" value="Completed Data Request"  onclick="openPage('anaystComplete.jsp')"/></li>
</ul>
</div>
</div>
...
<div class="admin">
<h2 align="center" style="color:blue"><i>Analyst (<%= session.getAttribute("userName").toString() %>) : Welcome to e-Data Request of Analytics </i></h2>
</div>
<br/>
<div align="center">
<%
...
try{
result=stmt.executeQuery("SELECT SUM(PENDING), SUM(RAISED), SUM(ASSIGNED), SUM(COMPLETE) FROM(SELECT ANALYST, NVL(CASE WHEN STATUS=0 THEN COUNT(FILEID) END,0) PENDING,  NVL(CASE WHEN STATUS=1 THEN COUNT(FILEID) END,0) RAISED, NVL(CASE WHEN STATUS=2 THEN COUNT(FILEID) END,0) ASSIGNED, NVL(CASE WHEN STATUS=3 THEN COUNT(FILEID) END,0) COMPLETE FROM REQUESTS WHERE TRIM(ANALYST)=TRIM('"+id+"') GROUP BY STATUS, ANALYST)");
%>
