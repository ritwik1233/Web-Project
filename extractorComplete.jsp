...
try{
result=stmt.executeQuery("SELECT FILEID ,TOPIC ,b.NAME ,c.NAME ,DESCRIPTION, EXT ,CASE WHEN STATUS=0 THEN 'Pending' WHEN STATUS=1 THEN 'Raised' WHEN STATUS=2 THEN 'Assigned' ELSE 'Complete' END, GEN_DT, STRT_DT, END_DT, d.NAME FROM REQUESTS a, USERS_A b, USERS_L c, USERS_E d WHERE a.ANALYST=b.ID AND a.LEADER=c.ID AND a.EXTRACTOR=d.ID AND STATUS=3 AND TRIM(EXTRACTOR)=TRIM('"+id+"') ORDER BY FILEID");
...
<div id="menu">
<ul>
<li><input type="button" class="btn" value="Homepage" onclick="openPage('extractorHome.jsp')"/></li>
</ul>
</div>
...
<td bgcolor="green" style="color:white"><%= result.getString(7) %></td>
</tr>
<% }
con.close();
stmt.close();
