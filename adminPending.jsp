...
try{
result=stmt.executeQuery("SELECT DISTINCT FILEID ,TOPIC ,b.NAME ,c.NAME ,DESCRIPTION, EXT ,CASE WHEN STATUS=0 THEN 'Pending' WHEN STATUS=1 THEN 'Raised' WHEN STATUS=2 THEN 'Assigned' ELSE 'Complete' END, GEN_DT FROM REQUESTS a, USERS_A b, USERS_L c WHERE a.ANALYST=b.ID AND a.LEADER=c.ID AND STATUS=1 ORDER BY FILEID");
...
<div id="menu">
<ul>
<li><input type="button" class="btn" value="Homepage" onclick="openPage('adminHome.jsp')"/></li>
</ul>
</div>
...
<td>
<form method="post" action="adminSubmit.jsp">
<select name="extractor">
<% 
 
try{
	
	resultset1=stmt1.executeQuery("SELECT ID, NAME FROM USERS WHERE POWER=3");
...
<input type="hidden" value="<%= result.getString(1) %>" name="fileid">
