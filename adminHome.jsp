...
try{
result1=stmt1.executeQuery("SELECT ID FROM USERS WHERE POWER=2");
...
<li><input type="button" class="btns" value="Search by Date"  onclick="openPage('adminSearch.jsp')"></li>
<li><input type="button" class="btns" value="Search by Extractor"  onclick="openPage('adminSearchE.jsp')"></li>
</ul>
</div>
</div>
...
<ul>
<li>
<input type="button" class="btns" value="Pending with Admin" onclick="openPage('adminPending.jsp')" />
</li>
<li>
<input type="button" class="btns" value="Assigned to Extractor"  onclick="openPage('adminAssigned.jsp')" />
</li>
<li><input type="button" class="btns" value="Completed Data Request"  onclick="openPage('adminComplete.jsp')"/></li>
</ul>
</div>
</div>
...
<div class="admin">
<h2 align="center" style="color:blue"><i>Administrator (<%= session.getAttribute("userName").toString() %>) : Welcome to e-Data Request of Analytics </i></h2>
</div>
<br/>
<div align="center">
<%
...
try{
result=stmt.executeQuery("SELECT SUM(PENDING), SUM(RAISED), SUM(ASSIGNED), SUM(COMPLETE) FROM(SELECT ANALYST, NVL(CASE WHEN STATUS=0 THEN COUNT(FILEID) END,0) PENDING,  NVL(CASE WHEN STATUS=1 THEN COUNT(FILEID) END,0) RAISED, NVL(CASE WHEN STATUS=2 THEN COUNT(FILEID) END,0) ASSIGNED, NVL(CASE WHEN STATUS=3 THEN COUNT(FILEID) END,0) COMPLETE FROM REQUESTS GROUP BY STATUS, ANALYST)");
%>
